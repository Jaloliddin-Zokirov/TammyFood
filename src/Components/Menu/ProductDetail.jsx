...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [id]); // `id` o'zgarganda API chaqiriladi
  ...
  return (
    <section className="mt-[40px] mb-[200px]">
      <div className="container">
        {/* Ma'lumotlar mavjud bo'lsa */}
        <h2 className="text-[#3C3C3C] text-[36px] font-bold text-center mb-[40px]">
          {id}
        </h2>
        <ul className="flex items-center gap-[20px] justify-between flex-wrap">
          {displayedData?.map((el) => {
            return (
              <li
                className="w-full max-w-[420px] flex items-start gap-5 rounded-md p-4 shadow-md"
                key={el.idMeal}
              >
                <img
                  ...
                  alt={el.strMeal}
                />
                <div className="flex flex-col flex-1 p-2 h-[180px]">
                  <h3 className="text-lg font-semibold">{el.strMeal}</h3>
                  <div className="flex items-center justify-between gap-4 mt-auto">
                    <button className="px-4 py-2 bg-[#FCC647] text-white rounded-md cursor-pointer">
                      Buy Now
                    </button>
                    <button>
                      <img
